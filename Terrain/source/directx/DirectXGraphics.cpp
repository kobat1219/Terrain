...
// 初期処理
bool DirectXGraphics::Init(HWND hWnd, unsigned int Width, unsigned int Height, bool fullscreen)
{
	HRESULT	hr = S_OK;
   ...
   // 機能レベル
	...
	hr = m_lpDevice->CreateRasterizerState(&rasterDesc, &m_rasterSolidState);
	if (FAILED(hr)) {
		return false;
	}
	...
	// create the rasterrizer state from the description we just filled out 
	hr = m_lpDevice->CreateRasterizerState(&rasterDesc, &m_rasterWireState);
	if (FAILED(hr)) {
		return false;
	}
	...
	m_lpImmediateContext->DSSetSamplers(0, 1, m_samplerstate.GetAddressOf());
...
void DirectXGraphics::TurnOnAlphaBlending()
{
	float blendFactor[4];
...
void DirectXGraphics::TurnOffAlphaBlending()
{
	float blendFactor[4];
