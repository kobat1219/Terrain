...
public:
	...
	bool Init() {
		...
		ID3D11Device* device = DirectXGraphics::GetInstance()->GetDXDevice();
	...
	void SetTransform(TYPE type,DirectX::XMFLOAT4X4& matrix) {
		...
		case TYPE::WORLD:
			...
			devicecontext->HSSetConstantBuffers(0, 1, &m_pConstantBufferWorld);
		...
		case TYPE::VIEW:
			...
			devicecontext->HSSetConstantBuffers(1, 1, &m_pConstantBufferView);
		...
		case TYPE::PROJECTION:
			...
			devicecontext->HSSetConstantBuffers(2, 1, &m_pConstantBufferProjection);
	...
	DirectX::XMMATRIX Projection() const{ return m_CBProjection.Projection; }
	DirectX::XMMATRIX View() const{ return m_CBView.View; }
protected:
