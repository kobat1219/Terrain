#include	<vector>
#include	"../../package/directxtex/include/DirectXTex.h"
#include	"Shader.h"
...
// ハルシェーダーオブジェクトを生成する
//--------------------------------------------------------------------------------------
bool CreateHullShader(ID3D11Device* device, const char* szFileName, LPCSTR szEntryPoint, LPCSTR szShaderModel, ID3D11HullShader** ppHullShader)
{
	HRESULT hr;
...
//--------------------------------------------------------------------------------------
bool CreateDomainShader(ID3D11Device* device, const char* szFileName, LPCSTR szEntryPoint, LPCSTR szShaderModel, ID3D11DomainShader** ppDomainShader)
{
	HRESULT hr;
...
//--------------------------------------------------------------------------------------
bool CreateComputeShader(ID3D11Device* device, const char* szFileName, LPCSTR szEntryPoint, LPCSTR szShaderModel, ID3D11ComputeShader** ppComputeShader)
{
	HRESULT hr;
...
bool CreateConstantBufferWrite(
	...
	HRESULT hr = device->CreateBuffer(&bd, nullptr, pConstantBuffer);
	if (FAILED(hr)) {
		MessageBox(nullptr, "CreateBuffer(constant buffer write) error", "Error", MB_OK);
		return false;
	}
...
bool CreateIndexBufferWrite(
	...
	HRESULT hr = device->CreateBuffer(&bd, &InitData, pIndexBuffer);
	if (FAILED(hr)) {
		MessageBox(nullptr, "CreateBuffer(index buffer write) error", "Error", MB_OK);
		return false;
	}
...
bool CreateVertexBufferWrite(
	...
	hr = device->CreateBuffer(&bd, &InitData, pVertexBuffer);		// バッファ生成
	if (FAILED(hr)) {
		MessageBox(nullptr, "CreateBuffer(vertex buffer write) error", "Error", MB_OK);
		return false;
	}
